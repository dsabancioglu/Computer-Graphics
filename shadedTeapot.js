...
var indices = []; // To keep the indices of triangle vertices ,ÇİZİLECEK ÜÇGENLERİN İNDİSLERİNİ TUTUYOR
...
var textureCoords = [	//texture mapping te kullanıyoruz
	2.00,2.00,0.00,
	1.50,2.00,0.00,
	1.50,1.95,0.00,
	2.00,1.95,0.00,
...
window.onload = function init() {
	...
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(normals), gl.STATIC_DRAW); //float32array= float (32 bitini ifade ediyor ama önemsiz)
    ...
    gl.bufferData( gl.ARRAY_BUFFER, new Float32Array(vertices), gl.STATIC_DRAW); //vertices array'i GPU ya gönderiliyor.
	...
	var iBuffer = gl.createBuffer();  	//indices arrayi için buffer oluşturduk
	gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, iBuffer); //DİKKT EDERSEN BU BİR ELEMENT ARRAY BUFFER (YANİ İNDİS TUTAN BUFFER)
	gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint8Array(indices), gl.STATIC_DRAW); //BU BUFFERDA TUTULAN DATALAR KULLANILARAK RENDER DA DRAWELEMENTS ŞEKLİNDE ÇİZDİRİYORUZ.
												//uint8array=unsigned int (indis arrayindeki değerler hep pozitif olacak)
...
function prepareTeapot() {
	for (var i = 0; i < quads.length; i+=12) { //satır satır atlamak için i+12 yapıyoruz
		indices.push(quads[i]-1, quads[i+3]-1, quads[i+6]-1); //çizdireceğim ilk üçgenin indislerini aldım
		indices.push(quads[i]-1, quads[i+6]-1, quads[i+9]-1);  //çizdireceğim 2.üçgenin indislerini aldım
	}
	for (var i = 0; i < triangles.length; i+=9) { //1 satırda 9 eleman olduğu için i+9 yazdık (satır atlamak için)
		indices.push(triangles[i]-1, triangles[i+3]-1, triangles[i+6]-1); //çizdireceğim üçgenlerin indisini aldım.
	}
}
...
function render() {
    ...
    gl.drawElements( gl.TRIANGLES, indices.length, gl.UNSIGNED_BYTE, 0 ); //wireframe yapısını görebilmek için line loop la çalıştırdık
}