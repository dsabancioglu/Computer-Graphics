...
function triangle(a,b,c) {
	points.push(a);		//points arrayine a ,b,c noktalarını ekledim.
	points.push(b);
	points.push(c);
}

function divideTriangle(a,b,c,count) { //count= level of recursion that we want
...
window.onload= function init() { 
	...
	var vertices = [ vec2(-1.0, -1.0), //vertices arrayini 2 boyutlu (x,y) koordinatlarıyla dolduruyorum.
					vec2(0.0, 1.0),
					vec2(1.0, -1.0) ];
					
	points= []; //points i empty array ile initialize ediyorum.
...
*/
	...
	//Böylece initialization kısmıyla render kısmını (3 kısımdan oluşuyodu ->initialization,geometric arrangements,rendering) ayırmış oldum.
		...
		gl.drawArrays( gl.TRIANGLES, 0, points.length); //içi dolu üçgen çizdirme		//points arrayinin length i kadar üçgen çizdirme
	}
